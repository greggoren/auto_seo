...
def write_competitors_quality_table(results):
    ...
    f.write("\\hline\n")
    f.write("Active competitors - single bot & "+" & ".join([results[i]["2"] for i in sorted(list(results.keys()))])+" \\\\ \n")
    f.write("\\hline\n")
    f.write("Active competitors - multiple bots & "+" & ".join([results[i]["0"] for i in sorted(list(results.keys()))])+" \\\\ \n")
    f.write("\\hline\n")
    f.write("\\end{tabular}\n")
    f.close()
