...
from CrossValidationUtils.rankSVM_crossvalidation import get_average_query_rank_promotion
from Crowdflower.check_effect_on_seo_with_different_ranks import retrieve_scores,create_trec_eval_file,create_features_for_different_ranks,read_chosen_model_file
...
def run_chosen_model_for_stats(chosen_models, method, qrels_file, feature_file, doc_name_index, seo_scores,base_features_file,ref_index,beta=""):
    ...
    increase_stats = get_average_query_rank_promotion(seo_scores, final_trec_file)
    similarities = read_similarity_file("/home/greg/auto_seo/scripts/similarities_file")
    add = ""
    if beta:
        add="_""_"+str(beta)
    table_name = "summary_corr_"+method+str(ref_index)+add+".text"
    create_correlation_for_different_ranks(similarities,increase_stats,table_name,ref_index)
    return table_name
...
def read_similarity_file(file):
    ...
    with open(file) as f:
        for line in f:
            query=line.split()[0]
            rank=line.split()[1]
            similarity=float(line.split()[2].rstrip())
            if query not in results:
                results[query]={}
            results[query][rank]=similarity
    return results
...
def write_corr_table(vecs_gain,vecs_sim,table_name,index):
    ...
    # if spearmanr(vec_sim,vecs_gain)[1] <= 0.05:
    #     s_corr = str(s_corr) + "^*"
    # else:
    #     s_corr = str(s_corr)
...
def create_correlation_for_different_ranks(similarities,average_gain,table_name,index):
    vecs_sim={i:[] for i in ["2","3","4"]}
    vecs_gain=[]
    for query in similarities:
        for i in similarities[query]:
            vecs_sim[i].append(similarities[query][i])
        vecs_gain.append(average_gain[5][query])
    write_corr_table(vecs_gain,vecs_sim,table_name,index)




def write_correlation_results(weighted_results_file,filename,beta,method,flag=False,last=False):
        ...
        for j,line in enumerate(file_w):
            if not flag:
                if j==0:
                    ...
                    f.write("\\hline\n")
                if j==1:
                    f.write("$ \\beta $ & "+line.upper().rstrip()+"\\\\ \n")
                    f.write("\\hline\n")
                if j==2:
...
if __name__=="__main__":
    ...
    feature_file_rank_2, doc_name_index_2, seo_score_2 = create_features_for_different_ranks(original_features_file_2,
                                                                                             1, 1,
                                                                                             original_qrels_file_2,
                                                                                             coherency_features)
    # print("creating features rank 3")
    # feature_file_rank_3, doc_name_index_3, seo_score_3 = create_features_for_different_ranks(original_features_file_3,
    #                                                                                          2, 2,
    #                                                                                          original_qrels_file_3,
    #                                                                                          coherency_features)
    # print("creating features rank 4")
    # feature_file_rank_4, doc_name_index_4, seo_score_4 = create_features_for_different_ranks(original_features_file_4,
    #                                                                                          3, 3,
    ...
    for beta in betas:
        ...
        if beta == betas[-1]:
            last = True
        summary_file_2 = run_chosen_model_for_stats(chosen_models, method, original_qrels_file_2, feature_file_rank_2,
                                                    doc_name_index_2,
                                                    seo_score_2, new_features_with_harmonic_file, "2")

        # summary_file_3 = run_chosen_model_for_stats(chosen_models, method, original_qrels_file_3, feature_file_rank_3,
        #                                             doc_name_index_3,
        #                                             seo_score_3, new_features_with_harmonic_file, "3")
        #
        # summary_file_4 = run_chosen_model_for_stats(chosen_models, method, original_qrels_file_4, feature_file_rank_4,
        #                                             doc_name_index_4,
        #                                             seo_score_4, new_features_with_harmonic_file, "4")
        write_weighted_results(summary_file_2, "summary_corr" + method.split("_")[0] + "_2.tex", beta, "RankSVM", flag,
                               last)
        # write_weighted_results(summary_file_3, "summary_corr" + method.split("_")[0] + "_3.tex", beta, "RankSVM", flag,
        #                        last)
        # write_weighted_results(summary_file_4, "summary_corr" + method.split("_")[0] + "_4.tex", beta, "RankSVM", flag,
        #                        last)
        flag = True
